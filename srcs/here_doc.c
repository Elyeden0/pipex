...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/07 15:24:50 by abonnard          #+#    #+#             */
/*   Updated: 2025/03/13 11:20:47 by abonnard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	here_doc(t_pipex *pipex, char **av)
{
	...
	pipex->infile_fd = open(HERE_DOC, O_CREAT | O_RDWR | O_TRUNC, 0666);
...
void	is_here_doc(t_pipex *pipex, char **av, int ac)
{
	...
	{
		pipex->outfile_fd = open(av[ac - 1], O_CREAT | O_RDWR | O_APPEND, 0666);
	...
	else
	{
		...
		pipex->outfile_fd = open(av[ac - 1], O_CREAT | O_RDWR | O_TRUNC, 0666);
		if (pipex->outfile_fd < 0)
			update_perror_status(pipex, av, ac);
